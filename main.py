...
class SubDownInc:
    ...
    def __init__(self, arg_settings: dict, arg_auth: dict):
        self.settings = arg_settings
        self.auth = arg_auth

        # Setup all the user settings
        self.mkvmerge_file_path = self.settings["mkvmerge-path"]
        self.language = self.settings["languages"]
        self.bool_delete_original_movie_after_task = self.settings["replace-original-file-after-task-done"]
        self.bool_delete_srt_file_after_task = self.settings["delete-subtitle-file-after-task-done"]
        self.prefix_for_subtitled_movie = self.settings["subtitled_movie_prefix"]
        self.bool_use_bearer_auth = self.settings["use-bearer-auth"]
        self.log_file_path = self.settings["error-file_path"]
        self.bool_softcode_subtitle = self.settings["softcode_subtitle"]


        # Setup all credentials
        self.api_key = self.auth["api-key"]
        self.uname = self.auth["username"]
        self.upass = self.auth["password"]
    ...
    def saveErrorsToLog(self, error_msg: str, full_path_movie: str):
        print(error_msg)
    ...
    def processMovie(self, full_path_movie: str):
        ...
        try:
            ...
            self.saveErrorsToLog(str(e), full_path_movie)
...
def main():
    ...
    try:
        ...
        print("Eroare - fisierul config\\settings.json nu a putut fi deschis - " + str(e))
        exit(1)
    ...
    try:
        ...
        print("Eroare - fisierul config\\auth.json nu a putut fi deschis - " + str(e))
        exit(1)
    ...
    # Create object
    sdi = SubDownInc(settings_dict, auth_dict)
